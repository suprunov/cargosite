$('.footer__top-list').each(function () {
  var list = $(this);
  var slider = $(
    '<div class="footer__top-slider"><div class="swiper"><div class="swiper-wrapper"></div></div><div class="swiper-scrlbr"></div></div>',
  );
...
$('.footer__nav-heading').on('click', function () {
  ...
  if (item.hasClass('_active')) {
    item.removeClass('_active').next('ul').slideUp(600, 'easeOutExpo');
  } else {
    $('.footer__nav-heading._active').trigger('click');
    item.next('ul').slideDown(600, 'easeOutExpo');
