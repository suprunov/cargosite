$.fn.required = function (opts) {
  ...
  $(this).each(function () {
      ...
      input
        ...
        .on('check:required', function () {
          ...
          if (input.prop('tagName') == 'SELECT') {
            ...
            input.hasClass('_inn') &&
            (input.val().length != 10 || input.val().length != 12) &&
            !input.data('success')
          ) {
            valid = false;
          } else if (
            input.hasClass('textInput-placeholder') ||
            input.val().length < min ||
          ...
          if (valid) {
            ...
            if (input.attr('type') == 'file') {
              ...
              input
                .parent()
                .after(
                  '<div class="form__input-error-message">' + input.data('message') + '</div>',
                );
            } else {
              input.after(
                '<div class="form__input-error-message">' + input.data('message') + '</div>',
    ...
    f.on('reset.clear', function () {
      ...
      f.find('input[type="text"], input[type="tel"], input[type="email"], textarea').each(
        function () {
